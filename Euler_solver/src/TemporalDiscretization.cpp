...
std::vector<std::vector<double>> thomasAlgorithm(std::vector<double>& a, // subdiagonal
    ...
    for (int i = 1; i < n; i++) {
        ...
        cp[i] = c[i] / m;
        // std::cout << "cp[i]: " << cp[i] << "c[i]: " << c[i] << " m: " << m << std::endl;
        for (int j = 0; j < numRHS; j++) {
    ...
    for (int j = 0; j < numRHS; j++) {
        ...
        for (int i = n - 2; i >= 0; i--) {
            ...
            // std::cout << "x[i][j]: " << x[i][j] << " dp[i][j]: " << dp[i][j] << " cp[i]: " << cp[i] << " x[i+1][j]: " << x[i+1][j] << std::endl;
        }
    }
...
std::vector<std::vector<double>> TemporalDiscretization::compute_dt() const {
    ...
    for (size_t j = 2; j < ny - 2; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            ...
            // std::cout << "dt[j][i]: " << dt[j][i] << std::endl;
...
std::tuple<std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>> TemporalDiscretization::compute_abc() const {
    ...
    for (size_t i = 0; i < nx; ++i) {
        for (size_t j = 2; j < ny - 2; ++j) {
            ...
            // std::cout << "a_J: " << a_J[i*nx + (j-2)] << " b_J: " << b_J[i*nx + (j-2)] << " c_J: " << c_J[i*nx + (j-2)] << std::endl;
...
std::tuple<std::vector<std::vector<std::vector<double>>>,
    ...
    while (it < it_max) {
        if (res_smoothing == 0) {
            ...
            for (int j = 2; j < ny - 2; ++j) {
                for (int i = 0; i < nx; ++i) {
                    ...
                    // std::cout << Rd0[0] << " " << Rd0[1] << " " << Rd0[2] << " " << Rd0[3] << std::endl;
                    ...
                    // std::cout << "current_state.W[j][i] " << current_state.W[j][i][0] << " " << current_state.W[j][i][1] << " " << current_state.W[j][i][2] << " " << current_state.W[j][i][3] << std::endl;
            ...
            for (int j = 2; j < ny - 2; ++j) {
                for (int i = 0; i < nx; ++i) {
                    ...
                    // std::cout << "R_d0 " << current_state.R_d0[j-2][i][0] << " " << current_state.R_d0[j-2][i][1] << " " << current_state.R_d0[j-2][i][2] << " " << current_state.R_d0[j-2][i][3] << std::endl;
                    // std::cout << "R_d " << current_state.R_d[j-2][i][0] << " " << current_state.R_d[j-2][i][1] << " " << current_state.R_d[j-2][i][2] << " " << current_state.R_d[j-2][i][3] << std::endl;
                    ...
                    // std::cout << "R_d " << current_state.R_d[j-2][i][0] << " " << current_state.R_d[j-2][i][1] << " " << current_state.R_d[j-2][i][2] << " " << current_state.R_d[j-2][i][3] << std::endl;
                    // std::cout << "D_1 " << current_state.D[j][i][0][0] << " " << current_state.D[j][i][0][1] << " " << current_state.D[j][i][0][2] << " " << current_state.D[j][i][0][3]; 
                    // std::cout << " D_4 " << current_state.D[j][i][1][0] << " " << current_state.D[j][i][1][1] << " " << current_state.D[j][i][1][2] << " " << current_state.D[j][i][1][3] << std::endl;
                    // std::cout << "R_d0 " << current_state.R_d0[j-2][i][0] << " " << current_state.R_d0[j-2][i][1] << " " << current_state.R_d0[j-2][i][2] << " " << current_state.R_d0[j-2][i][3] << std::endl;
                    // std::cout << "Rd20 " << Rd20[0] << " " << Rd20[1] << " " << Rd20[2] << " " << Rd20[3] << std::endl;
                    // std::cout << "dW " << dW[0] << " " << dW[1] << " " << dW[2] << " " << dW[3] << std::endl;
                }
            }
            current_state.run_odd();
            ...
            // for (size_t j = 2; j < ny - 2; ++j) {
            //     for (size_t i = 0; i < nx; ++i) {
            //         // std::cout << a_I[(j-2)*nx + i] << " " << b_I[(j-2)*nx + i] << " " << c_I[(j-2)*nx + i] << std::endl;
            //         std::cout << "d " << d[(j-2)*nx + i][0] << " " << d[(j-2)*nx + i][1] << " " << d[(j-2)*nx + i][2] << " " << d[(j-2)*nx + i][3] << std::endl;
            //     }
            // }
            ...
            // for (size_t j = 2; j < ny - 2; ++j) {
            //     for (size_t i = 0; i < nx; ++i) {
            //         std::cout << "Rs " << R_star[(j-2)*nx + i][0] << " " << R_star[(j-2)*nx + i][1] << " " << R_star[(j-2)*nx + i][2] << " " << R_star[(j-2)*nx + i][3] << std::endl;
            //     }
            // }
            ...
            // for (size_t j = 2; j < ny - 2; ++j) {
            //     for (size_t i = 0; i < nx; ++i) {
            //         std::cout << "Rstar reshape " << R_star[(j-2)*nx + i][0] << " " << R_star[(j-2)*nx + i][1] << " " << R_star[(j-2)*nx + i][2] << " " << R_star[(j-2)*nx + i][3] << std::endl;
            //     }
            // }
            std::vector<std::vector<double>> R_star_star = thomasAlgorithm(a_J, b_J, c_J, R_star);
            // for (size_t j = 2; j < ny - 2; ++j) {
            //     for (size_t i = 0; i < nx; ++i) {
            //         std::cout << "Rstarstar reshape " << R_star_star[(j-2)*nx + i][0] << " " << R_star_star[(j-2)*nx + i][1] << " " << R_star_star[(j-2)*nx + i][2] << " " << R_star_star[(j-2)*nx + i][3] << std::endl;
            //     }
            // }
        ...
        for (const auto &norm : l2_norm) {
            std::cout << norm << " ";
        }
        std::cout << "C_L:" << C_L << " C_D:" << C_D << " C_M:" << C_M << std::endl;

        // Check for convergence
        if (*std::max_element(l2_norm.begin(), l2_norm.end()) <= 1e-11) {
