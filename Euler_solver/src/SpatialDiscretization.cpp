...
// #pragma omp parallel for
    for (int j = 0; j < ny - 1; ++j) {
        for (int i = 0; i < nx - 1; ++i) {
            ...
            // std::cout << "OMEGA_domain[j][i]: " << OMEGA_domain[j][i] << std::endl;
            ...
            // std::cout << "n_domain[j][i][0]: " << n_domain[j][i][0][0] << " " << n_domain[j][i][0][1] << std::endl;
            n_domain[j][i][1] = {s_domain[j][i][1][0] / Ds_domain[j][i][1], s_domain[j][i][1][1] / Ds_domain[j][i][1]};
            // std::cout << "n_domain[j][i][1]: " << n_domain[j][i][1][0] << " " << n_domain[j][i][1][1] << std::endl;
...
void SpatialDiscretization::compute_dummy_cells() {
    ...
    for (int i = 0; i < nx - 1; ++i) {
        ...
        // std::cout << "W[0][i]: " << W[0][i][0] << " " << W[0][i][1] << " " << W[0][i][2] << " " << W[0][i][3] << std::endl;
        // std::cout << "rho_val: " << rho_val << " u_val: " << u_val << " v_val: " << v_val << " E_val: " << E_val << " T_val: " << T_val << " p2: " << p2 << std::endl;
        // std::cout << "u_dummy: " << u_dummy << " v_dummy: " << v_dummy << std::endl;
...
std::vector<double> SpatialDiscretization::FcDs(const std::vector<double>& W, const std::vector<double>& n, const double& Ds) const {
    ...
    // std::cout << "rho: " << rho << " u: " <<? u << " v: " << v << " E: " << E << " T: " << T << " p: " << p << std::endl;
    double V = n[0]*u + n[1]*v;
    // std::cout << "n[0]: " << n[0] << " n[1]: " << n[1] << std::endl;
...
void SpatialDiscretization::compute_Fc_DeltaS() {
    ...
    for (size_t j = 2; j < ny - 1; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            ...
            // std::cout << "avg_W1: " << avg_W1[0] << " " << avg_W1[1] << " " << avg_W1[2] << " " << avg_W1[3] << std::endl;
            std::vector<double> avg_W4 = vector_scale(0.5, vector_add(W[j][i], W[j][(i - 1 + nx) % nx]));
            // std::cout << "avg_W4: " << avg_W4[0] << " " << avg_W4[1] << " " << avg_W4[2] << " " << avg_W4[3] << std::endl;

            std::vector<double> FcDs_1 = FcDs(avg_W1, n[j][i][0], Ds[j][i][0]);
            // std::cout << "FcDs_1: " << FcDs_1[0] << " " << FcDs_1[1] << " " << FcDs_1[2] << " " << FcDs_1[3] << std::endl;
            std::vector<double> FcDs_4 = FcDs(avg_W4, n[j][i][1], Ds[j][i][1]);

            flux[j][i][0] = FcDs_1;
            // std::cout << "flux[j][i][0]: " << flux[j][i][0][0] << " " << flux[j][i][0][1] << " " << flux[j][i][0][2] << " " << flux[j][i][0][3] << std::endl;
            flux[j][i][1] = FcDs_4;
            // std::cout << "flux[j][i][1]: " << flux[j][i][1][0] << " " << flux[j][i][1][1] << " " << flux[j][i][1][2] << " " << flux[j][i][1][3] << std::endl;
        }
    }
...
std::tuple<double, double> SpatialDiscretization::compute_epsilon(const std::vector<double>& W_Im1, const std::vector<double>& W_I,
    ...
    // std::cout << "p_Im1: " << p_Im1 << " p_I: " << p_I << " p_Ip1: " << p_Ip1 << " p_Ip2: " << p_Ip2;
    // std::cout << " Gamma_I: " << Gamma_I << " Gamma_Ip1: " << Gamma_Ip1;
    // std::cout << " eps2: " << eps2 << " eps4: " << eps4 << std::endl;
...
void SpatialDiscretization::compute_lambda() {
    ...
    for (size_t j = 0; j < ny-1; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            ...
            // std::cout << "u " << u << " v " << v << " sx " << sx << " sy " << sy << std::endl;
            const double speci = std::abs(u_dot_n) + std::sqrt(cc*(sx*sx+sy*sy));
            Lambda_I[j][i] = speci;
            // std::cout << "Lambda_I[j][i]: " << Lambda_I[j][i] << std::endl;
        }
    }
    // #pragma omp parallel for
    for (size_t j = 0; j < ny-1; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            ...
            Lambda_J[j][i] = specj;
            // std::cout << "Lambda_J[j][i]: " << Lambda_J[j][i] << std::endl;
        }
    }
}
...
void SpatialDiscretization::compute_dissipation() {
    ...
    for (size_t j = 2; j < ny - 1; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            ...
            // std::cout << "Lambda_1_S: " << Lambda_1_S << " Lambda_4_S: " << Lambda_4_S << std::endl;
            ...
            // std::cout << "eps2_1: " << eps2_1 << " eps4_1: " << eps4_1 << " eps2_4: " << eps2_4 << " eps4_4: " << eps4_4 << std::endl;
            ...
            // std::cout << "D_1: " << D_1[0] << " " << D_1[1] << " " << D_1[2] << " " << D_1[3] << std::endl;
            // std::cout << "D_4: " << D_4[0] << " " << D_4[1] << " " << D_4[2] << " " << D_4[3] << std::endl;
    ...
    for (size_t i = 0; i < nx; ++i) {
        ...
        D[3][i][0] = vector_scale(Lambda_S[3][i][0],
            vector_subtract(
                vector_scale(eps_2[1][i][0], vector_subtract(W[2][i], W[3][i])),

                vector_scale(eps_4[1][i][0], vector_subtract(vector_subtract(
                    vector_scale(2.0, W[3][i]), W[2][i]), W[4][i]))
            )
        );
        // std::cout << "D[3][i][0]: " << D[3][i][0][0] << " " << D[3][i][0][1] << " " << D[3][i][0][2] << " " << D[3][i][0][3];

        // std::cout << " Lambda_S[3][i][0]: " << Lambda_S[3][i][0];
        // std::cout << " eps_2[3][i][0]: " << eps_2[1][i][0];
        // std::cout << " eps_4[3][i][0]: " << eps_4[1][i][0] << std::endl;

        // Calculate D_1 for cell (2, i)
        D[2][i][0] = vector_scale(Lambda_S[2][i][0],
            vector_subtract(
                vector_scale(eps_2[0][i][0], vector_subtract(W[2][i], W[3][i])),

                vector_scale(eps_4[0][i][0], vector_subtract(vector_subtract(
                    vector_scale(2.0, W[3][i]), W[2][i]), W[4][i]))
            )

        );
        // std::cout << "D[2][i][0]: " << D[2][i][0][0] << " " << D[2][i][0][1] << " " << D[2][i][0][2] << " " << D[2][i][0][3] << std::endl;

        // std::cout << "Lambda_S[2][i][0]: " << Lambda_S[2][i][0] << std::endl;
        // std::cout << "eps_2[2][i][0]: " << eps_2[2][i][0] << std::endl;
        // std::cout << "eps_4[2][i][0]: " << eps_4[2][i][0] << std::endl;
    }
}
...
void SpatialDiscretization::compute_R_c() {
    ...
    for (size_t j = 2; j < ny - 2; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            ...
            // std::cout << "FcDS_4: " << FcDS_4[0] << " " << FcDS_4[1] << " " << FcDS_4[2] << " " << FcDS_4[3] << std::endl;


            R_c[j-2][i] = vector_add(vector_add(FcDS_1, FcDS_2), vector_add(FcDS_3, FcDS_4));
            // std::cout << "R_c[j-2][i]: " << R_c[j-2][i][0] << " " << R_c[j-2][i][1] << " " << R_c[j-2][i][2] << " " << R_c[j-2][i][3] << std::endl;
...
void SpatialDiscretization::compute_R_d() {
    ...
    for (size_t j = 2; j < ny - 2; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            ...
            // std::cout << "R_d[j-2][i]: " << R_d[j-2][i][0] << " " << R_d[j-2][i][1] << " " << R_d[j-2][i][2] << " " << R_d[j-2][i][3] << std::endl;
